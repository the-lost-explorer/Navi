...
/**
 * Renders the 3D model continuously
 */
public class RendererRunnable implements Runnable {
    ...
    private void placeObject(Anchor anchor) {
        ((Activity) this.context).runOnUiThread(() -> ModelRenderable.builder()
                .setSource(RendererRunnable.this.context, Uri.parse("arrow.sfb"))
                .build()
                .thenAccept(renderable -> addNodeToScene(anchor, renderable))
                .exceptionally(
                        throwable -> {
                            Toast toast =
                                    Toast.makeText(RendererRunnable.this.context, "Unable to load andy renderable", Toast.LENGTH_LONG);
